...
export default function PreprocessSection({ stats }) {
  ...
  const [error, setError] = useState(null); // YENİ: Gerçek hataları tutacağımız state
  ...
  const runPreprocess = async () => {
    setLoading(true);
    setSuccess(false);
    setError(null); // YENİ: Her yeni denemede eski hatayı temizle
    ...
    try {
      ...
      // DÜZELTİLDİ: Artık hata alınca "success" demiyor, hatayı yakalıyor!
      console.error("Backend Error Details:", err);
      setError(
        err.response?.data?.detail || 
        err.message || 
        "Backend preprocessing failed. Check console for details."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: "2rem", alignItems: "start", animation: 'fadeIn 0.4s ease-out' }}>
      ...
      {/* SAĞ SÜTUN: GÖRSELLER VE SONUÇ EKRANI */}
      <div style={styles.combinedCard}>
        ...
        {/* YENİ: Kırmızı Hata Banner'ı */}
        {error && (
          <div style={{ backgroundColor: "#fef2f2", color: "#b91c1c", padding: "1.2rem 1.8rem", borderTop: "1px solid #fecaca", display: "flex", alignItems: "center", gap: "15px", animation: "slideUp 0.4s ease-out" }}>
            <div style={{ width: "24px", height: "24px", backgroundColor: "#dc2626", color: "white", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", fontSize: "0.9rem" }}>!</div>
            <div style={{ fontSize: "0.85rem", lineHeight: "1.4" }}>
              <b>Processing Failed:</b> {error}
            </div>
          </div>
        )}

        {/* Mevcut Yeşil Başarı Banner'ı */}
        {success && !error && (
