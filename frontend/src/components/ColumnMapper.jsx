...
export default function ColumnMapper({ onSaved, onCancel, previewData }) {
  const [targetColumn] = useState("readmitted_30d");
  ...
  const columns = [
  { name: "readmitted_30d", role: "target", auto: "Binary (0/1)" },
  { name: "age", role: "number", auto: "Number" },
  { name: "ejection_fraction", role: "number", auto: "Number" },
  { name: "serum_creatinine", role: "number", auto: "Number" },
  { name: "serum_sodium", role: "number", auto: "Number" },
  { name: "smoker", role: "category", auto: "Category" },
  { name: "patient_id", role: "ignore", auto: "Identifier-like" },
];
  ...
  const ROLE_DISPLAY = {
    ...
    ignore: "Ignore (not clinical)",
  };

  const saveMapping = async (shouldClose) => {
  setSaving(true);
  setError("");
  try {
    await API.post("/data/map-columns", {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>Assign roles to columns to prevent the model from learning from non-clinical data.</p>
      </div>

      {error && (
        <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca', color: '#b91c1c', padding: '1rem', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.85rem' }}>
          ⚠️ {error}
        </div>
      )}
          ...
          <div style={styles.card}>
            ...
            <div style={styles.alertBox}>
              ...
              <p style={{ margin: 0, fontSize: '0.85rem' }}>
                <b>Note:</b> Blood_Pressure has been automatically parsed to use the <b>Systolic</b> value.
              </p>
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => saveMapping(false)} disabled={saving} style={styles.btnSecondary}>
                Validate Schema
              </button>
              <button onClick={() => saveMapping(false)} disabled={saving} style={styles.btnTeal}>
                {justSaved ? "✓ Saved!" : saving ? "Saving..." : "Save Mapping"}
              </button>
            </div>
          </div>
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {Object.entries(previewData || {}).map(([key, val]) => (
                  <tr key={key}><td style={styles.td}>{key}</td><td style={styles.td}>{String(val)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <div style={styles.footer}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button onClick={() => saveMapping(true)} disabled={saving} style={styles.btnPrimary}>
